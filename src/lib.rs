...
pub struct Executor {
    /// Tasks as a smallvec to try to keep it in the stack for as long as
    /// possible
...
impl Woke for Task {
    fn wake_by_ref(_: &Arc<Self>) {
        // poll everything because future is done and may have created
        // conditions for something to finish
        Executor::run()
    }
}
...
impl Executor {
    ...
    /// For every current task, iterate through each one and poll them, if they
    /// are not done add them to end of list. If they are done, remove them from
    /// list of tasks.
